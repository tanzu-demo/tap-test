...
```
tanzu apps workload create serverless -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/serverless-source \
  --type web
```
...
```
tanzu apps workload get serverless
```

You can assign the URL to a variable to make it easier to reference it:

```
URL=$(kubectl get services.serving.knative.dev/serverless -o=jsonpath='{.status.url}')
 ```
